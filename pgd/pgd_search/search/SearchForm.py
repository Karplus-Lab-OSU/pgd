...
class SearchFormBase(forms.Form):
    ...
    resolutionMax   = forms.FloatField(required=False, min_value=0, initial=1.2, widget=forms.TextInput(attrs={'size':3}))
    rfactorMin      = forms.FloatField(required=False, min_value=0, initial=0, widget=forms.TextInput(attrs={'size':3}))
    rfactorMax      = forms.FloatField(required=False, min_value=0, initial=0.25, widget=forms.TextInput(attrs={'size':3}))
    rfreeMin        = forms.FloatField(required=False, min_value=0, initial=0, widget=forms.TextInput(attrs={'size':3}))
...
for i in RESIDUE_INDEXES:
    ...
    form_dict["ome_%i" % i]             = SearchSyntaxField(initial='<=-90,>=90', required=False, widget=forms.TextInput(attrs={'class':'field', 'size':8}))
    form_dict["ome_i_%i" % i]           = forms.IntegerField(required=False, widget=forms.HiddenInput(attrs={'class':'include'}))
    form_dict["omep_%i" % i]             = SearchSyntaxField(initial='<=-90,>=90', required=False, widget=forms.TextInput(attrs={'class':'field', 'size':8}))
    form_dict["omep_i_%i" % i]           = forms.IntegerField(required=False, widget=forms.HiddenInput(attrs={'class':'include'}))
