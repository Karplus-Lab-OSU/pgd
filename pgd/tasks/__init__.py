...
def register(key, task):
    TASK_MANAGER.register(key, task)
...
def autodiscover():
    ...
    for app in settings.INSTALLED_APPS:
        # For each app, we need to look for an task.py inside that app's
        # package. We can't use os.path here -- recall that modules may be
        # imported different ways (think zip files) -- so we need to get
        # the app's __path__ and look for tasks.py on that path.
        ...
        # Step 2: use imp.find_module to find the app's tasks.py. For some
